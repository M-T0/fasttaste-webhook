...
app.post('/api', (req, res) => {
  ...
  try {
    ...
    // Lấy giỏ hàng từ context. Đây là cách hoạt động đúng của Dialogflow ES.
    let cart = parameters.cart || [];
    let fulfillmentMessages = [];
    let outputContexts = [];

    // --- XỬ LÝ LOGIC THEO TỪNG INTENT ---

    // KỊCH BẢN 1: THÊM MÓN VÀO GIỎ HÀNG
    if (intentName === 'muathem') {
      ...
      if (newItems.length > 0) {
        ...
        // Cách lưu giỏ hàng chuẩn: Gửi lại giỏ hàng trong parameters của output context
        outputContexts.push({
